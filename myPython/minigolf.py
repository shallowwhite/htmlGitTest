...
class ChipShopDetector:
    ...
    def search_chip_shop(self, location):
        params = {
            'location': location,
            'radius': 500000,  # Searching within a 500km radius
            #'type': 'restaurant',
            'keyword': 'outdoor mini golf',
            'key': self.api_key,
        }
        ...
        for place in data.get('results', []):
            if place.get('user_ratings_total', 0) >= 300 and place.get('rating', 0) >= 4.2: #set  parameters of min user ratings total and minimum stars
                result = {
                    'name': place['name'],
...
if __name__ == "__main__":
    ...
    current_location = "52.66361183859692, -1.198140176088649"  # Enter you coords
