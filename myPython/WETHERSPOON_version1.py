...
class ChipShopDetector:
    ...
    def search_chip_shop(self, location):
        params = {
            'location': location,
            'radius': 100000,  # Searching within a 10km radius
            #'type': 'restaurant',
            'keyword': 'jd wetherspoon',
            'key': self.api_key,
        }
        ...
        for place in data.get('results', []):
            if place.get('user_ratings_total', 0) >= 200 and place.get('rating', 0) >= 4.2: #set  parameters of min user ratings total and minimum stars
                result = {
                    'name': place['name'],
...
if __name__ == "__main__":
    ...
    current_location = "54.4868561809427, -0.6063331073430571"  # Enter you coords
