...
class ChipShopDetector:
    ...
    def search_chip_shop(self, location):
        params = {
            'location': location,
            'radius': 40000,  # Searching within a 10km radius
            #'type': 'restaurant',
            'keyword': 'mot',
            'key': self.api_key,
        }
        ...
        for place in data.get('results', []):
            if place.get('user_ratings_total', 0) >= 100 and place.get('rating', 0) >= 4.2: #set  parameters of min user ratings total and minimum stars
                result = {
                    'name': place['name'],
...
if __name__ == "__main__":
    ...
    current_location = "53.421810590973465, -1.4554482804273134"  # Enter you coords
